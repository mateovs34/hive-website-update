...
const ALL_PRS = [
  { id: 145, title: "Pedido 24 geles — Pack Entrenamiento",    agent: "CABA",           status: "review",  comments: 2,  additions: 24,  deletions: 0,  branch: "orden-145", time: "Ahora" },
  { id: 144, title: "Pedido 6 geles — Pack Iniciación",        agent: "GBA Sur",        status: "review",  comments: 1,  additions: 6,   deletions: 0,  branch: "orden-144", time: "hace 1m" },
  { id: 143, title: "Suscripción mensual — 12 geles",          agent: "Córdoba",        status: "merged",  comments: 4,  additions: 12,  deletions: 0,  branch: "suscripcion", time: "hace 1m" },
  { id: 142, title: "Pedido mayorista — Club Atlético",        agent: "Mendoza",        status: "merged",  comments: 3,  additions: 100, deletions: 0,  branch: "mayorista", time: "hace 2m" },
  { id: 141, title: "Pedido 6 geles — Mix sabores",            agent: "Rosario",        status: "approved",comments: 1,  additions: 6,   deletions: 0,  branch: "orden-141", time: "hace 8m" },
  { id: 140, title: "Pedido 48 geles — Equipo Triatlón",       agent: "Mar del Plata",  status: "review",  comments: 5,  additions: 48,  deletions: 0,  branch: "triatlon", time: "hace 22m" },
  { id: 139, title: "Reposición stock — HIVE Ultra",           agent: "Patagonia",      status: "merged",  comments: 7,  additions: 200, deletions: 0,  branch: "reposicion", time: "hace 1h" },
]

const ALL_REVIEW_FILES = [
  { file: "stock/clásico.ts",     pct: 72 },
  { file: "stock/energía+.ts",    pct: 45 },
  { file: "stock/recovery.ts",    pct: 88 },
  { file: "stock/ultra.ts",       pct: 31 },
  { file: "envíos/pendientes.ts", pct: 60 },
]

const ALL_REVIEW_LINES: { type: "comment"|"approve"|"change"|"code"; text: string; author?: string }[] = [
  { type: "code",    text: 'const pedido = await orden.procesar(id)' },
  { type: "comment", text: "Stock confirmado para CABA", author: "logística" },
  { type: "code",    text: 'return envio.preparar(pedido, destino)' },
  { type: "approve", text: "Listo — envío confirmado", author: "depósito" },
  { type: "code",    text: 'export const despachar = async (orden) => {' },
  { type: "change",  text: "Agregar tracking automático", author: "sistemas" },
  { type: "code",    text: '  const pack = await stock.reservar(items)' },
  { type: "approve", text: "Aprobado — a enviar", author: "control" },
  { type: "code",    text: '  await notificar.cliente(estado)' },
  { type: "comment", text: "Enviar confirmación por email", author: "atención" },
  { type: "code",    text: 'return { status: "despachado", tracking }' },
  { type: "approve", text: "Todo listo para entrega", author: "logística" },
]

const COMMITS = [
  { hash: "a3f8c21", msg: "nuevo: pedido procesado #145",          time: "Ahora" },
  { hash: "b7d2e09", msg: "envío: despachado a Córdoba",           time: "hace 4m"   },
  { hash: "c9a1f34", msg: "stock: actualizado HIVE Ultra",         time: "hace 12m"  },
  { hash: "d4e6b78", msg: "suscripción: renovación automática",    time: "hace 31m"  },
  { hash: "e2c9d56", msg: "mayorista: pedido club confirmado",     time: "hace 1h"   },
]
...
function StatusBadge({ status }: { status: string }) {
  const cfg = {
    merged:   { bg: "rgba(217,119,6,0.12)",  color: "#b45309", icon: <GitMerge style={{ width: 9, height: 9 }} />,  label: "Enviado"   },
    approved: { bg: "rgba(245,158,11,0.12)",   color: "#d97706", icon: <CheckCircle2 style={{ width: 9, height: 9 }} />, label: "Aprobado" },
    review:   { bg: "rgba(251,191,36,0.15)",color: "#b45309", icon: <Eye style={{ width: 9, height: 9 }} />,       label: "Pendiente"},
...
function ReviewLine({ item, delay }: { item: typeof REVIEW_LINES[0]; delay: number }) {
  ...
  const iconCfg = {
    approve: { icon: <CheckCircle2 style={{ width: 9, height: 9, color: "#d97706", flexShrink: 0 }} />, color: "#d97706" },
    change:  { icon: <AlertCircle  style={{ width: 9, height: 9, color: "#f59e0b", flexShrink: 0 }} />, color: "#f59e0b" },
    ...
    >
      <div style={{
        ...
        <div style={{
          ...
          }}>hive / dashboard — producción</span>
          <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 8 }}>
            <LiveDot />
            <span style={{ fontSize: 8, color: "rgba(40,167,69,0.8)", letterSpacing: "0.16em", fontFamily: "monospace" }}>ENVÍOS ACTIVOS</span>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", borderBottom: "1px solid rgba(0,0,0,0.06)", background: "rgba(251,250,247,0.9)" }}>
          {[
            { label: "Geles vendidos",     val: 18,       icon: <GitMerge style={{ width: 11, height: 11 }} />,   graph: <MiniBarGraph seed={0} /> },
            { label: "Atletas activos",   val: 34,       icon: <Eye style={{ width: 11, height: 11 }} />,         graph: <MiniBarGraph seed={5} /> },
            { label: "Entrenamientos",    val: 127,      icon: <GitCommit style={{ width: 11, height: 11 }} />,   graph: <MiniDotGraph seed={2} /> },
            { label: "Energía / hr",      val: reqCount, icon: <Zap style={{ width: 11, height: 11 }} />,         graph: <LiveSparkline seed={7} /> },
          ].map((m, i) => (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.1fr 1fr 0.85fr", gap: 8, padding: 8, height: 340, overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 5, height: "100%", overflow: "hidden", ...anim(160) }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 2px", flexShrink: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                ...
                <span style={{ fontSize: 8.5, letterSpacing: "0.13em", textTransform: "uppercase", color: "rgba(0,0,0,0.38)", fontFamily: "monospace" }}>Pedidos</span>
              </div>
              <span style={{ fontSize: 7.5, color: "rgba(0,0,0,0.25)", fontFamily: "monospace" }}>{ALL_PRS.filter(p => p.status === "review").length} PENDIENTES</span>
            </div>
                ...
                <div key={`${pr.id}-${prOffset}`} style={{
                  ...
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <div style={{ display: "flex", gap: 7 }}>
                      <span style={{ fontSize: 7.5, color: "#d97706", fontFamily: "monospace" }}>+{pr.additions}</span>
                      <span style={{ fontSize: 7.5, color: "#92400e", fontFamily: "monospace" }}>-{pr.deletions}</span>
            ...
            <div style={{ ...panel, padding: "8px 10px", flexShrink: 0, height: 76, overflow: "hidden" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 5, marginBottom: 6 }}>
                ...
                <span style={{ fontSize: 7.5, letterSpacing: "0.13em", textTransform: "uppercase", color: "rgba(0,0,0,0.33)", fontFamily: "monospace" }}>Actividad de Envíos</span>
              </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 5, height: "100%", overflow: "hidden", ...anim(210) }}>
            <div style={{ display: "flex", alignItems: "center", gap: 5, padding: "0 2px", flexShrink: 0 }}>
              ...
              <span style={{ fontSize: 8.5, letterSpacing: "0.13em", textTransform: "uppercase", color: "rgba(0,0,0,0.38)", fontFamily: "monospace" }}>Detalle Pedido — #{ALL_PRS[reviewFileIdx % ALL_PRS.length].id}</span>
            </div>
            <div style={{ ...panel, flex: 1, padding: "9px 10px", overflow: "hidden", display: "flex", flexDirection: "column" }}>
              ...
              <div style={{ marginBottom: 7, paddingBottom: 7, borderBottom: "1px solid rgba(0,0,0,0.05)", flexShrink: 0 }}>
                ...
                <div style={{ display: "flex", gap: 5 }}>
                  <span style={{ fontSize: 7.5, color: "#d97706", fontFamily: "monospace" }}>+142</span>
                  <span style={{ fontSize: 7.5, color: "#92400e", fontFamily: "monospace" }}>-27</span>
                ...
                {ALL_REVIEW_FILES.map((f, i) => (
                  <div key={f.file} style={{ opacity: i === reviewFileIdx ? 1 : 0.55, transition: "opacity 0.4s ease" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
                      ...
                      <span style={{ fontSize: 7.5, fontFamily: "monospace", color: reviewFilePcts[i] > 70 ? "#d97706" : "#b45309", transition: "color 0.4s ease", fontWeight: i === reviewFileIdx ? 700 : 400 }}>{Math.round(reviewFilePcts[i])}%</span>
                    </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 5, height: "100%", overflow: "hidden", ...anim(260) }}>
            <div style={{ display: "flex", alignItems: "center", gap: 5, padding: "0 2px", flexShrink: 0 }}>
              ...
              <span style={{ fontSize: 8.5, letterSpacing: "0.13em", textTransform: "uppercase", color: "rgba(0,0,0,0.38)", fontFamily: "monospace" }}>Últimos Pedidos</span>
            </div>
              ...
              {COMMITS.slice(0, 4).map((c, i) => (
                <div key={c.hash} style={{
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <span style={{ fontSize: 7.5, fontFamily: "monospace", color: "#b45309" }}>{c.hash}</span>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 5, padding: "2px 2px 0", flexShrink: 0 }}>
              ...
              <span style={{ fontSize: 8.5, letterSpacing: "0.13em", textTransform: "uppercase", color: "rgba(0,0,0,0.38)", fontFamily: "monospace" }}>Stock / Productos</span>
            </div>
            <div style={{ ...panel, flexShrink: 0, overflow: "hidden" }}>
              {[
                { name: "stock-clásico", status: "passing", duration: "1m 32s" },
                { name: "stock-energía",    status: "running", duration: "0m 48s" },
                { name: "stock-recovery",   status: "passing", duration: "2m 11s" },
                { name: "stock-ultra",    status: "running", duration: "0m 54s" },
              ].map((a, i) => (
                <div key={a.name} style={{
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    {a.status === "running"
                      ...
                      : <CheckCircle2 style={{ width: 8, height: 8, color: "#d97706", flexShrink: 0 }} />
                    }
